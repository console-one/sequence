/**
 * tool-auto-wiring.test.ts — tool invocations fire like derived values
 * when the match is unambiguous.
 *
 * The kernel has long made a distinction between derived values (schema
 * carries `derived(...)`, cascade fires automatically) and tool
 * invocations (gap exists + matching tool in backward inference, but the
 ...
 * between a gap's required type and the tool's output type, with input
 * values present at the paths named by the tool's param type's property
 * keys. When multiple tools could fill the gap, the kernel does NOT wire
 * — ambiguity is left to a handler at a higher scope.
 *
 * Calling convention: the tool impl is invoked with a single object arg
...
describe('tool auto-wiring — sole-match fires in cascade', () => {
  ...
  test('sole-match tool auto-fires when input paths have values (tool then gap)', () => {
    const seq = new Sequence(() => 1000);
    const { schema, impl } = mkDoubleCap();

    // Register the tool — fn-typed schema + fn bind.
    ...
    // Provide input at the tool's declared property path.
    seq.mount('bind', 'x', 21);

    // Cascade fires the tool and fills the gap.
    expect(seq.get('result.doubled')).toBe(42);
  });

  test('gap-then-tool order also wires and fires', () => {
    const seq = new Sequence(() => 1000);
    const { schema, impl } = mkDoubleCap();
    ...
    // Before the tool is registered, nothing to fire.
    expect(seq.get('result.doubled')).toBeUndefined();
  ...
  test('multi-match does NOT wire — ambiguity left for a handler', () => {
    ...
    // Two tools, both producing number from {x: number}.
    ...
    // Both tools match — kernel should not pick arbitrarily.
  ...
  test('missing-input does not fire — impl waits for all declared inputs', () => {
    ...
    // No input at path `x` — tool has no values to read.
    expect(seq.get('result.doubled')).toBeUndefined();
  });

  test('gap with a pre-existing value is NOT wired when the tool arrives', () => {
    const seq = new Sequence(() => 1000);
    const { schema, impl } = mkDoubleCap();

    // Schema + value land before the tool exists. Wiring skips this
