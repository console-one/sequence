...
interface Environment {
  ...
  saveSnapshot(projection: Readonly<Projection>, sinceSeq: number): void;

  /** Register this environment's capabilities onto the Sequence. */
  mountCapabilities(seq: Sequence): void;

  /** The process loop: pull from generator, push responses. */
  run(channel: Generator<string, void, string>): Promise<void>;
}
```

That's the contract. Four methods + a clock. Everything else is the Sequence.
...
function bootKernel(env: Environment): Generator<string, void, string> {
  ...
  // 3. Mount the environment's capabilities
  env.mountCapabilities(seq);

  // 4. Return the generator channel
  return channel(seq, env);
}
...
class ElectronEnvironment implements Environment {
  ...
  mountCapabilities(seq: Sequence): void {
    ...
    seq.mount('cap', 'ui.render', true); // external — renderer fills this
  }
...
class LambdaEnvironment implements Environment {
  ...
  mountCapabilities(seq: Sequence): void {
    ...
    seq.mount('schema', 'heartbeat', FT.number());
...
3. env.mountCapabilities → registers what this environment can do
4. bootKernel(env)       → creates Sequence, loads snapshot, mounts caps, returns generator
5. env.run(generator)    → pulls ft text (yield), pushes responses (next), handles ticks
6. env.saveSnapshot()    → persists projection periodically
```
