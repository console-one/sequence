...
- `sequence.ts` — Statement sequence with append, project, suspend, resume, cascade, compact
- `type.ts` — Constraint set with generic var (VarType) — NOT tested for backward propagation
- `builder.ts` — FT.* API
- `merge.ts` — Type checking (forward only)
- `tell.ts` — Constraint builders (eq, lt, exists)
- `hoist.ts` — Projection → text
- `process.ts` — createProcess = new Sequence

The plumbing works. The intelligence is entirely missing.
