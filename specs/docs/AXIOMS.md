...
## Status: 332+ tests, 0 compile errors
...
Blocks contain MountEntries. Entries are `{ op, path, value }`. Blocks have `where`, `while`, `status`. Status is IMMUTABLE — set once at creation.
- `statement.ts:Block`, `statement.ts:MountEntry`
...
- `sequence.ts:mount()` — creates immutable blocks
- `sequence.ts:checkWhileClauses()` — appends invalidation blocks
- `sequence.ts:tryResumeSuspended()` — creates new applied blocks

**A3'. Mount takes a block. Blocks are atomic.**
`mount(entries[], opts)` evaluates all entries against head at block start. Any failure → entire block suspended. Cascade fires once after the block.
- `sequence.ts:mount()` — block evaluation loop
- Tests: `process.test.ts` "mount — block-based (atomic)" suite
...
`mount()` is the only public operation. It appends a block AND returns the projected state. Cascade, resume, invalidation, conjunction propagation are internal consequences.
...
- `sequence.ts:mount(entries[], opts)`
...
`propagateConjunctionDelta(changedPath)` runs inside mount's cascade. Only touches conjunctions referencing the changed path. Updates probability cache and sibling priorities.
...
ft/
  ...
  classregistration/mount.ts  —  Consumer pattern
```
